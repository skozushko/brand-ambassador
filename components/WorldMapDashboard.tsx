...
// Keys are zero-padded 3-digit strings matching world-atlas geo.id
const ISO_TO_CONTINENT: Record<string, string> = {
  ...
  "239": "Africa", // South Georgia (remote, treat as Africa)
...
const CONTINENT_CONFIG: Record<string, {
  color: string
  hover: string
  centroid: [number, number]
}> = {
  "North America": { color: "#3B82F6", hover: "#2563EB", centroid: [-100, 48] },
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
          ...
          >
            <Geographies geography={GEO_URL}>
              {({ geographies }) =>
                geographies.map((geo) => {
                  ...
                  const continent = ISO_TO_CONTINENT[isoKey]
                  const cfg = continent ? CONTINENT_CONFIG[continent] : undefined
            ...
            {/* Count labels at continent centroids */}
            {stats && CONTINENTS.map((name) => {
              const count = stats.byContinent[name] ?? 0
              const cfg = CONTINENT_CONFIG[name]
              return (
                <Marker key={name} coordinates={cfg.centroid}>
                  <circle r={14} fill="white" fillOpacity={0.9} stroke={cfg.color} strokeWidth={1.5} />
                  <text
                    textAnchor="middle"
        ...
        <div className="w-44 flex-shrink-0 border-l border-gray-100 p-5">
          ...
          {/* "Other" row — only shown if any BAs have unmapped countries */}
